# Daily Review - 2026-07-13

Generated: 2026-07-14T03:45:11.754Z

## Executive Read
- Tracked picks reviewed: 1
- Settled picks: 0
- Win rate: -
- Average CLV: -1.6 pts
- Beat-close rate: 0.0%
- Average Brier: -
- Staked: $0.00
- Net P/L: $0.00

## Best Pick Signal
- Pick: **France**
- Match: France vs Spain
- Sport: Soccer
- Open: +135 at BetUS
- Latest/close: +144 at LowVig.ag
- Model probability: 44.3%
- CLV: -1.6 pts
- Result: pending

## All Tracked Picks
| Sport | Match | Pick | Result | Open | Latest/Close | CLV | Model % | Brier | Stake | P/L | Notes |
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| Soccer | France vs Spain | France | pending | +135 | +144 | -1.6 pts | 44.3% | - | - | - | open BetUS; latest LowVig.ag; auto-close |

## Sport Splits
| Group | Picks | Settled | Win % | Avg CLV | Beat Close | Avg Brier | ROI |
|---|---:|---:|---:|---:|---:|---:|---:|
| Soccer | 1 | 0 | - | -1.6 pts | 0.0% | - | - |

## Confidence Buckets
| Group | Picks | Settled | Win % | Avg CLV | Beat Close | Avg Brier | ROI |
|---|---:|---:|---:|---:|---:|---:|---:|
| Under 55% | 1 | 0 | - | -1.6 pts | 0.0% | - | - |

## Ready To Bet Gate Review
### Gate Performance
| Group | Picks | Settled | Win % | Avg CLV | Beat Close | Avg Brier | ROI |
|---|---:|---:|---:|---:|---:|---:|---:|
| Passed gate | 1 | 0 | - | -1.6 pts | 0.0% | - | - |

### Saved Gate Blockers
_No saved gate blockers on reviewed picks yet._

## What Went Wrong
_No settled misses in this review window._

## Daily Intel That Mattered
- **Soccer / Medium / Market move / pending:** Spain at France — Draw price is +215, which is short enough to demand draw-risk review before backing either side.
- **Soccer / Medium / Market move / pending:** Argentina at England — Draw price is +195, which is short enough to demand draw-risk review before backing either side.

## Algorithm Adjustments To Consider
- Do not increase stake sizing yet: fewer than half of tracked prices beat the close.

## Next Actions
- Run `npm run update:data` before making new picks.
- Track the pick before the game starts so CLV is meaningful.
- After finals, refresh once more so auto-grading can settle pending picks.

